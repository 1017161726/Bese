...
class TextButton @JvmOverloads constructor(
     ...
     * 是否带边框
     */
    private var borderEnable: Boolean = false
    ...
    init {
        ...
        cornerRadius =
            typedArray.getDimension(R.styleable.TextButton_radius, CORNER_RADIUS_DEF.toFloat())
        borderLineSize = typedArray.getDimension(
            R.styleable.TextButton_borderLineSize,
            BORDERLINE_SIZE_DEF.toFloat()
        )
        borderLineColor =
            typedArray.getColor(
                R.styleable.TextButton_borderLineColor,
                BORDERLINE_COLOR_DEF
            )
        bgColor = typedArray.getColor(
            R.styleable.TextButton_bgColor,
            BG_COLOR_DEF
        )
        bgColorPressed =
            typedArray.getColor(
                R.styleable.TextButton_bgColorPressed,
                BG_COLOR_PRESSED_DEF
            )
        shadowWidth = typedArray.getDimension(
            R.styleable.TextButton_shadowWidth,
            SHADOW_WIDTH_DEF.toFloat()
        )

        // TypedArray must be Recycle after used
        typedArray.recycle()

        borderEnable = borderLineSize > 0
    ...
    private fun updateBg(drawable: Drawable?) {
        if (drawable == null) {
            return
        }
        background = drawable
    ...
    companion object {
        ...
        private val SHADOW_WIDTH_DEF = 0
        private val BORDERLINE_SIZE_DEF = 0
        private val CORNER_RADIUS_DEF = 1
        private val BORDERLINE_COLOR_DEF = Color.TRANSPARENT
        private val BG_COLOR_DEF = Color.WHITE
        private val BG_COLOR_PRESSED_DEF = Color.TRANSPARENT
