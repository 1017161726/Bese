 ...
 * 弹窗工具
 *      效果：仿iOS系统弹窗界面
 *      适用范围：自定义弹窗可统一使用此工具，view传给diyView，其他布局隐藏
 *                          一般展示弹窗，不要求效果
 *      Link：增强弹窗  LDialog，不自定义View，在需要按钮样式增强时使用
 *
 */
class XDialog(private val mCtx: Context) {
     ...
     * 设置后提供的左右屏幕边距会无效
     */
    fun setDiyView(diyView: View): XDialog {
     ...
     */
    fun setDiyViewMarginTop(margin: Float): XDialog {
     ...
     * 设置Dialog的自定义View竖直间距
     */
    fun setDiyViewMarginBottom(margin: Float): XDialog {
     ...
     */
    fun setDiyViewMarginHorizontal(margin: Float): XDialog {
